...
public interface ILoadPrefabAsync
{
    ...
    public ELoadStatus LoadResult { get; set; }
    public ELoadTarget PrefabType { get; }
    ...
    {
        if (f_Target.LoadResult is ELoadStatus.Loading or ELoadStatus.Succeed)
        {
            return;
        }
        var lastLoadKey = GUtil.GetTempKey();
        f_Target.LoadKey = lastLoadKey;
        f_Target.LoadResult = ELoadStatus.Loading;

        var asset = await GManager.LoadGameObjectManager.LoadAsync(f_Target.PrefabType);
        if (lastLoadKey != f_Target.LoadKey 
            || f_Target.LoadResult is not ELoadStatus.Loading)
        {
            GManager.LoadGameObjectManager.UnLoad(f_Target.PrefabType);
            return;
        }
        var target = GameObject.Instantiate(asset);
        var result = target.GetComponent<UnityObjectPoolItem>();
        f_Target.LoadResult = ELoadStatus.Succeed;
        ...
        where TLoad : UnityObjectPoolItemData
    {
        ...
        if (f_Target.IsLoadComplete)
        {
            ...
            GameObject.Destroy(f_Target.PrefabTarget.gameObject);
            GManager.LoadGameObjectManager.UnLoad(f_Target.PrefabType);
            f_Target.ClearEntityTarget();
        }
        f_Target.UnLoad();
        f_Target.LoadResult = ELoadStatus.UnLoad;

    }
}
public abstract class UnityObjectPoolItemData : ILoadPrefabAsync, IPoolClass
{
    //--
    //===============================----------------------========================================
    //-----------------------------                          --------------------------------------
    ...
    //--
    public int LoadKey { get; set; }
    public abstract ELoadTarget PrefabType { get; }
    ...
    {
        if (LoadResult is ELoadStatus.Succeed or ELoadStatus.Loading)
        {
            return true;
        }
        return false;
